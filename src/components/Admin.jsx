    ...
    <Container fluid className="admin-panel-container py-4">
      ...
      <div className="admin-header mb-4">
        <Row className="align-items-center">
          <Col><h2 className="mb-0"><span className="text-danger">⚡</span> PMC GAMING Admin</h2><p className="text-muted mb-0">Manage your tutorials</p></Col>
          <Col className="text-end"><Button variant="outline-danger" onClick={handleLogout} className="px-4"><FaSignOutAlt className="me-2" /> Logout</Button></Col>
        </Row>
      </div>
        ...
        <Col lg={5} className="mb-4">
          <Card className="admin-form-card border-0 shadow-sm">
            <Card.Header className="bg-danger text-white py-3">
              <h4 className="mb-0">{editingId ? <><FaEdit className="me-2" /> Edit Tutorial</> : <><FaPlus className="me-2" /> Add New Tutorial</>}</h4>
            </Card.Header>
            <Card.Body className="p-4">
              <Form onSubmit={handleSubmit}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control as="textarea" name="description" value={formData.description} onChange={handleChange} placeholder="Describe what this tutorial covers..." rows={3} required className="py-2" />
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label className="fw-bold"><FaImage className="me-2 text-danger" /> Thumbnail URL</Form.Label>
                  <Form.Control type="text" name="thumbnail" value={formData.thumbnail} onChange={handleChange} placeholder="https://i.imgur.com/example.jpg" required className="py-2" />
                </Form.Group>
                <Form.Group className="mb-3">
          ...
          <Card className="admin-table-card border-0 shadow-sm">
            <Card.Header className="bg-dark text-white py-3">
              <Row className="align-items-center">
                ...
                <Col md={5}><Form.Control type="text" placeholder="🔍 Search tutorials..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-dark text-white border-secondary" /></Col>
              </Row>
            </Card.Header>
            <Card.Body className="p-0">
              {filteredTutorials.length === 0 ? (
                <div className="text-center py-5"><p className="text-muted mb-0">No tutorials yet. Add your first tutorial!</p></div>
              ) : (
                  ...
                  <Table hover className="admin-table mb-0">
                    ...
                    <tbody>
                      {filteredTutorials.map(tutorial => (
                        <tr key={tutorial.id}>
                          <td><img src={tutorial.thumbnail} alt={tutorial.title} style={{ width: '50px', height: '50px', objectFit: 'cover', borderRadius: '4px' }} onError={(e) => e.target.src = 'https://via.placeholder.com/50?text=No+Image'} /></td>
                          ...
                          <td className="text-center">
                            ...
                            <Button variant="danger" size="sm" className="px-3" onClick={() => handleDeleteClick(tutorial)}><FaTrash className="me-1" /> Delete</Button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
